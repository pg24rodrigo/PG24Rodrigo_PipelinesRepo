...
namespace RFerzuli.DungeonGenerator.Runtime
{
    public class DungeonGenerator : MonoBehaviour
    {
        ...
        public void AddRoom()
        {
            ...
            if (Rooms.Count >= 1)
            {
                center = Rooms[Rooms.Count - 1].center;
        ...
        public void GenerateAll()
        {
            if (_seed == 0)
            {
                Random.InitState(Random.Range(1, int.MaxValue));
            }
            else
            {
        ...
        private void PlacePrefabs(PlaceableSet set, string containerName)
        {
            foreach (Bounds room in Rooms)
            {
                ...
                for (int x = 0; x < tileCountX; x++)
                {
                    ...
                    {
                        Vector3 corner = roomCorner + new Vector3(x * sizeX, 0.0f, z * sizeZ);
                        ...
                        if (!room.Contains(prefabBounds.min) || !room.Contains(prefabBounds.max))
                        {
                            continue;
                        }
                        ...
                        {
                            Quaternion rotation =
                                Quaternion.LookRotation(_directions[Random.Range(0, _directions.Length)]);
                            GameObject tile = null;

#if UNITY_EDITOR
                            tile = UnityEditor.PrefabUtility.InstantiatePrefab(
                                set.Prefabs[Random.Range(0, set.Prefabs.Count)]) as GameObject;
#else
